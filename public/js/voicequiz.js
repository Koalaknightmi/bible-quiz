  ...
  if(started){
    if (quote) {
      if (v < str[c].length) {
        ...
        postData("/vquotecomplete",JSON.stringify({ch:ch,score:time,prompt:prompt,user:JSON.parse(localsave("userdata","get")).name,pw:JSON.parse(localsave("userdata","get")).pass}),data => {
      ...
      if (time > 0) {
        ...
        postData("/vpostcomplete",JSON.stringify({ch:ch,score:score,prompt:prompt,user:JSON.parse(localsave("userdata","get")).name,pw:JSON.parse(localsave("userdata","get")).pass}),data => {
        console.log(JSON.stringify(data))
        location.reload();
      })
      }
